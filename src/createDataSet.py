...
class Generator:
	def gen_data(self): #generates usable position pngs from a file with raw pngs
		f = open('../samples/lichess_db_standard_rated_2013-01.pgn')
	...
	def gen_val_data(self): #generates usable position pngs from a file with raw pngs
		f = open('../samples/lichess_db_standard_rated_2013-01_val.pgn')
	...
	def converter(self, pos, label):
		x = tf.dtypes.cast(pgnToTensor.PgnToTensor().pgn_to_tensor(str(pos)),tf.int32)
